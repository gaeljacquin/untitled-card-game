...
const app = new Elysia({ adapter: node() })
    ...
    origin: (request) => {
        ...
        if (Array.isArray(corsOrigin))
            return corsOrigin.includes(requestOrigin);
        return false;
    },
...
const corsOrigin = clientUrlsEnv.length > 0 ? clientUrlsEnv : (isDev ? true : []);
...
io.on("connection", (socket) => {
    ...
    socket.on("game-next-round", (payload) => {
        console.info(`Message received from client ${socket.id}:`, payload);
